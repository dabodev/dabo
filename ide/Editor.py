...
class EditorForm(dabo.ui.dForm):
	...
	def fillMenu(self):
		...
		fileMenu.prependSeparator()
		fileMenu.prepend(_("Save &As\tCtrl+Shift+S"), OnHit=self.onFileSaveAs, bmp="saveAs", 
				help=_("Save under a different file name"))
		fileMenu.prepend(_("&Save\tCtrl+S"), OnHit=self.onFileSave, bmp="save",
				help=_("Save file"))
		cPos = fileMenu.getItemIndex(_("Close Window"))
		if cPos is not None:
			fileMenu.remove(cPos)
		fileMenu.prepend(_("&Close Editor\tCtrl+W"), OnHit=self.onFileClose, bmp="close",
				help=_("Close file"))
		fileMenu.prepend(_("&Open\tCtrl+O"), OnHit=self.onFileOpen, bmp="open",
				help=_("Open file"))
		fileMenu.prepend(_("&New\tCtrl+N"), OnHit=self.onFileNew, bmp="new",
				help=_("New file"))

		editMenu.appendSeparator()
		editMenu.append(_("&Jump to line...\tCtrl+J"), OnHit=self.onEditJumpToLine, 
				bmp="", help=_("Jump to line"))
		editMenu.appendSeparator()
		editMenu.append(_("Co&mment Line\tCtrl+M"), OnHit=self.onCommentLine,
				bmp="", help=_("Comment out selection"))
		editMenu.append(_("&Uncomment Line\tCtrl+Shift+M"), 
				OnHit=self.onUncommentLine, bmp="", 
				help=_("Uncomme&nt selection"))
		editMenu.append(_("&AutoComplete\tF5"), 
		...
		editMenu.appendSeparator()
		self._wrapItem = editMenu.append(_("&Word Wrap\tCtrl+Shift+W"), OnHit=self.onWordWrap, 
				bmp="", help=_("Toggle WordWrap"), menutype="check")
		self._synColorItem = editMenu.append(_("S&yntax Coloring\tCtrl+Shift+Y"), 
				OnHit=self.onSyntaxColoring, bmp="", help=_("Toggle Syntax Coloring"), 
				menutype="check")
		
		runMenu.append(_("&Run Script\tF7"), OnHit=self.onRunScript,
				bmp="", help=_("Run Script"))		
		self._showOutItem = runMenu.append(_("Hide/Show Output\tF8"), OnHit=self.onOutput, bmp="",
		...
		fontMenu.append(_("Zoom &In\tCtrl++"), OnHit=self.onViewZoomIn, 
				bmp="zoomIn", help=_("Zoom In"))
		fontMenu.append(_("&Normal Zoom\tCtrl+/"), OnHit=self.onViewZoomNormal, 
				bmp="zoomNormal", help=_("Normal Zoom"))
		fontMenu.append(_("Zoom &Out\tCtrl+-"), OnHit=self.onViewZoomOut, 
