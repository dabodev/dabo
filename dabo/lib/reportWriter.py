...
def getFloatLeading(obj):
    ...
    adj_factor = 0.1  ## (amount to adjust for each "+" or "-")
...
class Groups(ReportObjectCollection):
    pass


class Objects(ReportObjectCollection):
    pass
...
class ReportObject(CaselessDict):
    ...
    def initAvailableProps(self):
        self.AvailableProps["Comment"] = toPropDict(
            str, "", """You can add a comment here, the report will ignore it."""
        )
    ...
    def setProp(self, prop, val, logUndo=True):
        ...
        if False:
            ...
            if defProp and self.getProp(prop, evaluate=False) == self.getProp(
                defProp, evaluate=False
            ):
...
class Drawable(ReportObject):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["Width"] = toPropDict(
            float, 55.0, """Specifies the width of the object."""
        )

        self.AvailableProps["Height"] = toPropDict(
            float, 18.0, """Specifies the height of the object."""
        )

        self.AvailableProps["Rotation"] = toPropDict(
            float, 0.0, """Specifies the rotation of the object, in degrees."""
        )

        self.AvailableProps["hAnchor"] = toPropDict(
            str,
            "left",
        ...
        self.AvailableProps["vAnchor"] = toPropDict(
            str,
            "bottom",
        ...
        self.AvailableProps["Show"] = toPropDict(
            bool,
            True,
...
class Report(ReportObject):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["Subject"] = toPropDict(
            str,
            "",
            """Specifies the subject of the report. Appears in PDF properties.""",
        )

        self.AvailableProps["Keywords"] = toPropDict(
            tuple,
            (),
            """Specifies keywords for the report. Appears in PDF properties.""",
        )

        self.AvailableProps["Title"] = toPropDict(
            str, "", """Specifies the title of the report. Appears in PDF properties."""
        )

        self.AvailableProps["ColumnCount"] = toPropDict(
            int, 1, """Specifies the number of columns to divide the report into."""
        )

        self.AvailableProps["ColumnPadding"] = toPropDict(
            float, 0, """Specifies how much space to leave in between columns."""
        )
...
class Page(ReportObject):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["MarginLeft"] = toPropDict(
            float, ".5 in", """Specifies the page's left margin."""
        )

        self.AvailableProps["MarginTop"] = toPropDict(
            float, ".5 in", """Specifies the page's top margin."""
        )

        self.AvailableProps["MarginRight"] = toPropDict(
            float, ".5 in", """Specifies the page's right margin."""
        )

        self.AvailableProps["Orientation"] = toPropDict(
            str,
            "portrait",
        ...
        self.AvailableProps["Size"] = toPropDict(
            str,
            "letter",
...
class Group(ReportObject):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["StartOnNewColumn"] = toPropDict(
            bool, False, _("""Specifies whether groups should begin on a new column.""")
        )

        self.AvailableProps["ReprintHeaderOnNewColumn"] = toPropDict(
            bool,
            False,
            _("""Specifies whether the group header gets reprinted on new columns."""),
        )

        self.AvailableProps["StartOnNewPage"] = toPropDict(
            bool, False, _("""Specifies whether groups should begin on a new page.""")
        )

        self.AvailableProps["ReprintHeaderOnNewPage"] = toPropDict(
            bool,
            False,
            _("""Specifies whether the group header gets reprinted on new pages."""),
        )

        self.AvailableProps["ResetPageNumber"] = toPropDict(
            bool,
            False,
...
class Variable(ReportObject):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["Name"] = toPropDict(
            str, None, """Specifies the name of the variable."""
        )

        self.AvailableProps["ResetAt"] = toPropDict(
            str,
            None,
...
class Band(ReportObject):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["TotalHeight"] = toPropDict(
            float,
            0.0,
...
class Rectangle(Drawable):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["StrokeWidth"] = toPropDict(
            float, 1, """Specifies the width of the stroke, in points."""
        )

        self.AvailableProps["StrokeColor"] = toPropDict(
            tuple, (0, 0, 0), """Specifies the stroke color."""
        )

        self.AvailableProps["StrokeDashArray"] = toPropDict(
            tuple,
            None,
...
class String(Drawable):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["BorderWidth"] = toPropDict(
            float, 0, """Specifies the width of the border around the string."""
        )

        self.AvailableProps["BorderColor"] = toPropDict(
            tuple, (0, 0, 0), """Specifies the border color."""
        )

        self.AvailableProps["Align"] = toPropDict(
            str,
            "left",
        ...
        self.AvailableProps["FontSize"] = toPropDict(
            float, 10, """Specifies the size of the font, in points."""
        )

        self.AvailableProps["FontColor"] = toPropDict(
            tuple, (0, 0, 0), """Specifies the color of the text."""
        )

        self.AvailableProps["ScalePercent"] = toPropDict(
            tuple,
            (100.0, 100.0),
...
class Memo(String):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["PadLeft"] = toPropDict(
            float, 0, """Specifies the padding on the left side of the frame."""
        )

        self.AvailableProps["PadRight"] = toPropDict(
            float, 0, """Specifies the padding on the right side of the frame."""
        )

        self.AvailableProps["PadTop"] = toPropDict(
            float, 0, """Specifies the padding on the top side of the frame."""
        )

        self.AvailableProps["PadBottom"] = toPropDict(
            float, 0, """Specifies the padding on the bottom side of the frame."""
        )
        ...
        self.AvailableProps["FirstLineIndent"] = toPropDict(
            float,
            0,
...
class BarGraph(Drawable):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["Labels"] = toPropDict(
            list, [], """Specifies the lables to display on the bottom."""
        )

        self.AvailableProps["Title"] = toPropDict(
            str, "Title", """Specifies the title to display on the graph."""
        )

        self.AvailableProps["XLabel"] = toPropDict(
            str, "X-Label", """Specifies the label to display on the x axis."""
        )

        self.AvailableProps["YLabel"] = toPropDict(
            str, "Y-Label", """Specifies the label to display on the y axis."""
        )

        self.AvailableProps["XGrid"] = toPropDict(
            bool,
            False,
            """Specifies if a grid should be displayed on the major ticks of the x axis.""",
        )

        self.AvailableProps["YGrid"] = toPropDict(
            bool,
            False,
        ...
        self.AvailableProps["Orientation"] = toPropDict(
            str, "horizontal", """Orientation of the graph (vertical, horizontal)."""
        )

        self.AvailableProps["Log"] = toPropDict(
            bool, False, """True sets orientation axis to log scale"""
        )

        self.AvailableProps["LabelTextSize"] = toPropDict(
            str, "x-small", """The size of the text to display for the labels."""
        )

        self.AvailableProps["BarColor"] = toPropDict(
            str, "blue", """Specifies the colour of the bars."""
        )

        self.AvailableProps["BarBorder"] = toPropDict(
            int, 0, """Specifies the size of the border of the bar."""
        )

        self.AvailableProps["BarBorderColor"] = toPropDict(
            str, "black", """Specifies the colour of the border of the bar."""
        )

        self.AvailableProps["Error"] = toPropDict(
            list, [], """Specifies the error bars to display on the graph."""
        )

        self.AvailableProps["CapSize"] = toPropDict(
            int, 3, """Determines the length in points of the error bar caps"""
        )

        self.AvailableProps["ErrorBarColor"] = toPropDict(
            str, "black", """Specifies the colour of the error bars."""
        )
        ...
        self.AvailableProps["BackgroundColor"] = toPropDict(
            str, "white", """Specifies the colour of the border of the bar."""
        )
...
class Line(Drawable):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["LineSlant"] = toPropDict(
            str,
            "-",
...
class SpanningLine(Line):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["yFooter"] = toPropDict(
            float,
            0.0,
...
class Frameset(Drawable):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["BorderWidth"] = toPropDict(
            float, 0, """Specifies the width of the frame border."""
        )
        ...
        self.AvailableProps["ColumnCount"] = toPropDict(
            int, 1, """Specifies the number of columns in the frame."""
        )
...
class Paragraph(Drawable):
    ...
    def initAvailableProps(self):
        ...
        self.AvailableProps["SpaceBefore"] = toPropDict(
            float, 0, """Specifies the amount of space to leave before the paragraph."""
        )

        self.AvailableProps["LeftIndent"] = toPropDict(
            float, 0, """Specifies the amount to indent from the left."""
        )
...
class ReportWriter(object):
    ...
    def getFramesets(self):
        ...
        for b in bandObjs:
            framesets += [
                (idx, f, b)
                for (idx, f) in enumerate(b["Objects"])
                if type(f) == Frameset
                and len(f["Objects"]) == 1
    ...
    def drawSpanningObjects(self, origin=(0, 0), group=None):
        ...
        for obj in spanList_page:
            y1 = self.getPt(obj.getProp("yFooter")) + self.getPt(
                self.ReportForm["page"].getProp("marginBottom")
            )
            x1 = self.getPt(obj.getProp("xFooter")) + x
            self.draw(obj, (x1, y1))
            ...
            if isinstance(fobject, (Memo, Paragraph)):
                ...
                for para in paras:
                    if len(para) == 0:
                        ...
                        splits = ParaClass(para, s).split(
                            columnWidth - padLeft - padRight, availableHeight
                        )
                        if not splits:
        ...
        if height is not None and neededHeight > height and not overrideExpr:
            if not story:
                return self.getStory(
                    obj, overrideExpr="<<< string too long >>>", overrideFontSize=7
                )
            else:
    ...
    def write(self, save=True):
        ...
        def printBand(band, y=None, group=None, deferred=None):
            ...
            if band.lower() != "pageheader" and _form["PageHeader"].getProp("ColumnBreakAfter"):
                ...
                pageHeaderOrigin = (
                    ml,
                    pageHeight - mt - self.getPt(_form["PageHeader"].getProp("Height")),
                )
            ...
            if band.lower() == "groupfooter" and bandDict.getProp("PrintAtBottom"):
                ...
                if groupFooterHeight is None:
                    raise ValueError(
                        "GroupFooter height must be fixed (not None) when PrintAtBottom is True."
                    )
            ...
            for idx, obj in enumerate(objects):
                ...
                new_obj_deferred, neededHeight = self.draw(
                    obj,
                    (x1, y1),
    ...
    def _getXMLFromForm(self, form):
        ...
        return dicttoxml(d, header=self._getXmlHeader(), linesep={1: os.linesep * 1})
            ...
            for child in xmldict["children"]:
                if child["name"].lower() == "testcursor":
                    ...
                    if child["name"].lower() in ("objects", "groups", "variables"):
                        ...
                        formdict[child["name"]] = self._getFormFromXMLDict(
                            child, reportObject, level + 1
                        )

        return formdict

    def _getReportObject(self, objectType, parent):
        typeMapping = CaselessDict(
            {
                "Report": Report,
                "Page": Page,
                "Group": Group,
