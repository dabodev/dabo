...
class dBizobj(dObject):
    ...
    def _beforeInit(self):
        ...
        self.deleteChildLogic = constants.REFINTEG_CASCADE  # child records will be deleted
        self.updateChildLogic = constants.REFINTEG_IGNORE  # parent keys can be changed w/o
        # affecting children
        self.insertChildLogic = constants.REFINTEG_IGNORE  # child records can be inserted
    ...
    def clear(self, confirmed=False):
        ...
        if not confirmed:
            raise ValueError(
                "Must pass call clearRecords(True) to "
    ...
    def _flushCursors(self, flush_changed=False, flush_current=False):
        ...
        for key, cursor in list(self.__cursors.items()):
            if (not flush_current and cursor is self._CurrentCursor) or (
                not flush_changed and cursor.isChanged()
            ):
                cursors[key] = cursor
    ...
    def createCursor(self, key=None, addToCursorCollection=True):
        ...
        if errMsg:
            raise exceptions.dException(errMsg)
    ...
    def saveAll(self, startTransaction=True, saveTheChildren=True):
        ...
        except (exceptions.DBQueryException, exceptions.dException):
            if startTransaction:
    ...
    def save(self, startTransaction=True, saveTheChildren=True):
        ...
        except (exceptions.ConnectionLostException, exceptions.NoRecordsException):
            raise
        except (exceptions.DBQueryException, exceptions.dException):
    ...
    def deleteAllChildren(self, startTransaction=True):
        ...
        except exceptions.DBQueryException:
            if startTransaction:
                self.rollbackTransaction()
    ...
    def delete(self, startTransaction=True, inLoop=False):
        ...
        if self.KeyField is None:
            raise exceptions.dException(_("No key field defined for table: ") + self.DataSource)

        if self.deleteChildLogic == constants.REFINTEG_RESTRICT:
    ...
    def requery(self, convertQMarks=False):
        ...
        if self.KeyField is None:
            ...
            raise exceptions.MissingPKException(errMsg)
        ...
        if not (
            ...
            except exceptions.ConnectionLostException:
                raise
            except exceptions.DBQueryException:
                raise
            except exceptions.NoRecordsException:
                # Pass the exception to the UI
                uiException = exceptions.NoRecordsException
            except exceptions.dException:
                raise
            self._visitedKeys.clear()
    ...
    def filter(self, fld, expr, op="="):
        ...
        if fld in self.VirtualFields:
            ...
            self._CurrentCursor.filterByExpression(
                "%s IN (%s)"
                % (
                    self.KeyField,
    ...
    def fieldValidation(self, fld, val):
        ...
        if message == constants.BIZ_DEFAULT_FIELD_VALID:
        ...
        if errMsg:
            ...
            raise exceptions.BusinessRulePassed
    ...
    def validateField(self, fld, val):
        ...
        from being changed.
        """
        return constants.BIZ_DEFAULT_FIELD_VALID
    ...
    def seek(
        ...
        """
        ret = self._CurrentCursor.seek(
            val, fld, caseSensitive, near, sort=sort, incremental=incremental
        )
        if ret != -1:
            if runRequery:
                self.requeryAllChildren()
    ...
    def addMMBizobj(self, mmBizobj, assocTable, assocPKColThis, assocPKColOther, mmPkCol=None):
        ...
        if mmBizobj.DataSource not in self._associations:
            ...
            crs._isMM = True
            crs.createAssociation(
                mmBizobj.DataSourceName,
                mmPkCol,
                assocTable,
                assocPKColThis,
                assocPKColOther,
    ...
    def setValues(self, valDict=None, row=None, pk=None, **kwargs):
        warnings.warn(
            _(
                "setValues() is deprecated, and will be removed in an "
                "upcoming version. Use setFieldVals() instead."
            ),
            DeprecationWarning,
            1,
        )
    ...
    def getDataTypeForField(self, fld):
        ...
        if not ds:
            return None
        try:
            fldInfo = [rec[1] for rec in ds if rec[0] == fld][0]
        except IndexError:
            return None
    ...
    def getChildByDataSource(self, dataSource):
        ...
        ret = [child for child in self.getChildren() if child.DataSource == dataSource]
        try:
            return ret[0]
        except IndexError:
    ...
    def mmAddToBoth(self, bizOrDS, thisField, thisVal, otherField, otherVal):
        ...
        return self._mmAssociatedDbCall(
            bizOrDS, "mmAddToBoth", thisField, thisVal, otherField, otherVal
        )
    ...
    def createAssociation(
        self, mmOtherTable, mmOtherPKCol, assocTable, assocPKColThis, assocPKColOther
    ):
        """
        ...
        """
        return self._CurrentCursor.createAssociation(
            mmOtherTable, mmOtherPKCol, assocTable, assocPKColThis, assocPKColOther
        )
...
displayed to the user."""
    ...
    beforePointerMove = _makeHookMethod("beforePointerMove", "the record pointer moves")
    ...
    afterNew = _makeHookMethod(
        "afterNew",
        "a new record is added",
        additionalDoc="""Use this hook to change field values of newly added records. If
...
""",
    ...
    afterChange = _makeHookMethod(
        "afterChange",
        "a record is changed",
        additionalDoc="""This hook will be called after a successful save() or delete(). Contrast
...
afterDelete() which is only called after a delete().""",
    ...
    @AutoPopulatePK.setter
    def AutoPopulatePK(self, val):
        self._autoPopulatePK = bool(val)
        self._syncWithCursors()

    @property
    def AutoQuoteNames(self):
        """
        When True (default), table and column names are enclosed with quotes during
        SQL creation in the cursor.  (bool)
        """
        return self._autoQuoteNames

    @AutoQuoteNames.setter
    def AutoQuoteNames(self, val):
    ...
    @CascadeDeleteFromParent.setter
    def CascadeDeleteFromParent(self, val):
        self._cascadeDeleteFromParent = bool(val)

    @property
    def ChildCacheInterval(self):
        """
    ...
    @ChildCacheInterval.setter
    def ChildCacheInterval(self, val):
    ...
    def DataSourceName(self, val):
        """
        If set, treated as cursor real table name where DataSource is an alias for it. This allows
        coexistence of many business objects with same data source on single form. (str)
    ...
    def DataStructure(self, val):
        ...
        Although API clearly states there should be tuple type value, to achieve backward
        compatibility with older code, list are explicitly converted into tuples.
        """
    ...
    def DefaultValues(self):
        ...
        The values of the dictionary can be literal (must match the field type), or they can be a
        function object which will be called when the new record is added to the bizobj.
        """
        return self._defaultValues

    @DefaultValues.setter
    def DefaultValues(self, val):
        self._defaultValues = val

    @property
    def Encoding(self):
        """Name of encoding to use for bytes to str  (str)"""
        try:
            ret = self._encoding
        except AttributeError:
            ret = None
    ...
    @FillLinkFromParent.setter
    def FillLinkFromParent(self, val):
    ...
    def KeyField(self):
        """
        Name of field that is the PK. If multiple fields make up the key, separate the
        fields with commas. (str)
        """
        try:
    ...
    @NewChildOnNew.setter
    def NewChildOnNew(self, val):
        self._newChildOnNew = bool(val)

    @property
    def NewRecordOnNewParent(self):
    ...
    @NewRecordOnNewParent.setter
    def NewRecordOnNewParent(self, val):
        self._newRecordOnNewParent = bool(val)

    @property
    def NonUpdateFields(self):
        """Fields in the cursor to be ignored during updates"""
        return self._CurrentCursor.getNonUpdateFields()

    @NonUpdateFields.setter
    def NonUpdateFields(self, fldList=None):
        if fldList is None:
            fldList = []
    ...
    @property
    def Parent(self):
        """Reference to the parent bizobj to this one. (dBizobj | None)"""
        try:
            return self._parent
        except AttributeError:
            return None
    ...
    @property
    def ParentLinkField(self):
        """
        Name of the field in the parent table that is used to determine child records. If empty,
        it is assumed that the parent's PK is used  (str)
        """
        try:
    ...
    def Record(self):
        """
        Represents a record in the data set. You can address individual columns by referring
        to 'self.Record.fieldName' (read-only) (no type)
        """
        try:
    ...
    @RequeryChildOnSave.setter
    def RequeryChildOnSave(self, val):
        self._requeryChildOnSave = bool(val)

    @property
    def RequeryChildrenOnNavigate(self):
        """
        Do we requery child bizobjs when navigating records? (bool)

        Usually, you want to automatically requery children when the parent RowNumber changes.
        However, there may be cases when you want to do that requery manually at the proper time
        instead for performance reasons.

        Setting this property to False will keep requeryAllChildren() from automatically running
        after RowNumber changes.
        """
        try:
    ...
    @RequeryChildrenOnNavigate.setter
    def RequeryChildrenOnNavigate(self, val):
        self._requeryChildrenOnNavigate = bool(val)

    @property
    def RequeryOnLoad(self):
        """
        When True, the cursor object runs its query immediately. This is useful for
        lookup tables or fixed-size (small) tables. (bool)
        """
        try:
    ...
    def RequeryOnLoad(self, val):
        self._requeryOnLoad = bool(val)

    @property
    def RequeryWithParent(self):
        """
    ...
    @RequeryWithParent.setter
    def RequeryWithParent(self, val):
        self._requeryWithParent = bool(val)

    @property
    def RestorePositionOnRequery(self):
    ...
    @RestorePositionOnRequery.setter
    def RestorePositionOnRequery(self, val):
        self._restorePositionOnRequery = bool(val)

    @property
    def RowCount(self):
        """
        The number of records in the cursor's data set. It will be -1 if the cursor hasn't
        run any successful queries yet. (int)
        """
        try:
    ...
    @SaveNewUnchanged.setter
    def SaveNewUnchanged(self, val):
        self._saveNewUnchanged = val

    @property
    def ScanRestorePosition(self):
        """
        After running a scan, do we attempt to restore the record position to where it was before
        the scan (True, default), or do we leave the pointer at the end of the recordset (False).
        (bool)
        """
        return self._scanRestorePosition

    @ScanRestorePosition.setter
    def ScanRestorePosition(self, val):
        self._scanRestorePosition = val

    @property
    def ScanRequeryChildren(self):
        """
        When calling the scan() function, this property determines if we requery any child bizobjs
        for each row in this bizobj. The default is False, as this has the potential to cause
        performance issues.    (bool)
        """
        return self._scanRequeryChildren

    @ScanRequeryChildren.setter
    def ScanRequeryChildren(self, val):
        self._scanRequeryChildren = val

    @property
    def ScanReverse(self):
        """Do we scan the records in reverse order? (Default: False) (bool)"""
        return self._scanReverse

    @ScanReverse.setter
    def ScanReverse(self, val):
    ...
    def VirtualFields(self):
        ...
        The specified function will be called when getFieldVal() is called on the specified virtual
        field name.

        We need to save the explicitly-assigned VirtualFields here in the bizobj, so that we are
        able to propagate it to any future-assigned child cursors.
        """
    ...
    @VirtualFields.setter
    def VirtualFields(self, val):
