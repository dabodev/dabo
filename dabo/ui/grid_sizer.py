    ...
    def setColExpand(self, expand, colNum, proportion=0):
        ...
        if isinstance(colNum, (list, tuple)):
            ...
            if colNum.lower() == "all":
                ...
                raise ValueError(
                    _(
                        f"Invalid value passed for 'colNum' parameter: '{colNum}'. Only column "
                        "numbers or the word 'all' are valid."
                    )
                )
        else:
    ...
    def setRowExpand(self, expand, rowNum, proportion=0):
        ...
        if isinstance(rowNum, (list, tuple)):
            ...
            if rowNum.lower() == "all":
                ...
                raise ValueError(
                    _(
                        f"Invalid value passed for 'rowNum' parameter: '{rowNum}'. Only row "
                        "numbers or the word 'all' are valid."
                    )
                )
        else:
    ...
    def setRowSpan(self, obj, rowspan):
        ...
        if rowspan > 1:
            if not self._clearCells(obj, rowspan, "row"):
                dabo_module.error(
                    f"Cannot set RowSpan for {itm.Name}; remove objects in the way first."
                )
                return
    ...
    def setColSpan(self, obj, colspan):
        ...
        if colspan > 1:
            if not self._clearCells(obj, colspan, "col"):
                dabo_module.error(
                    f"Cannot set ColSpan for {itm.Name}; remove objects in the way first."
                )
                return
