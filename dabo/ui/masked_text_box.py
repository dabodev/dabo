...
class dMaskedTextBox(dTextBoxMixin, masked.TextCtrl):
    ...
    def Format(self):
        """
        Several pre-defined formats are available. When you set the Format property, any Mask
        setting is ignored, and the specified Format is used instead. The format codes are NOT
        case-sensitive.  (str)
    ...
    def Value(self):
        """
        Specifies the content of this control. (str) If ValueMode is set to 'Masked', this will
        include the mask characters. Otherwise it will be the contents without any mask characters.
        """
    ...
    def ValueMode(self):
        """
        Specifies the information that the Value property refers to. (str) If it is set to 'Masked'
        (or anything that begins with the letter 'm'), the Value property will return the contents
        of the control, including any mask characters. If this is set to anything other than a
        string that begins with 'm', Value will return the control's contents without the mask
        characters.
        NOTE: This only affects the results of `reading` the Value property. Setting Value is not
        affected in any way.
        """
        try:
...
if __name__ == "__main__":
    ...
    class MaskedForm(dForm):
        def afterInit(self):
            ...
            pgf = dPageFrame(self, TabPosition="Left", PageCount=3)
            ...
            sz = pg1.Sizer = dGridSizer(MaxCols=2, HGap=5, VGap=5)

            lbl = dLabel(pg1, Caption="Basic Masks")
            lbl.FontSize += 2
...
versions of wxPython)""",
            ...
            sz.append(dLabel(pg1, Caption="Letters (any case) Only:"), halign="right")
            ...
            sz = pg2.Sizer = dGridSizer(MaxCols=2, HGap=5, VGap=5)

            lbl = dLabel(pg2, Caption="Pre-defined Formats")
            lbl.FontSize += 2
            sz.append(lbl, colSpan=2, halign="center")
