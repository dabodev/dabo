...
class dAutoComplete(dcm.dControlMixin, TextCtrlAutoComplete):
    ...
    """Creates a text box with a dropdown list that has the ability to filter multiple records or choices
    in the list based on entered text.
    ...
    def listFromDS(self):
	"""Takes a data set from the DataSource bizobj or the DataSet property directly
	and formats it as a choice list for autocomplete, using field names as column headers
	by default or the ColNames property as column headers if defined."""
	...
	"""The default entry callback function: takes the user-entered text from the text box
	and updates the dropdown list to include only entries in which the value of the search
	...
	if not text:
	    ...
	    choices = [choice for choice in self._dynamicChoices if choice[self._colSearch].lower().startswith(text)]
	    while len(choices) < 2:
	...
	if self._constructed():
		...
		raise ValueError("Cannot set Choices: A single or multi-column list with at least one element must be provided.")
    ...
    SearchField = property(_getSearchField, _setSearchField, None,
                           """The field or column used to compare with user-entered text.   (str or int)
                                ...
                                field's value.  If an int is provided, columns are chosen by index (starting with 0).
                                If not set, the search will be done on the leftmost field (index 0).""")
    
    FetchField = property(_getFetchField, _setFetchField, None,
                          """The field or column displayed in the text box when a value is selected from the list. (str or int)
                          
                                When given a field name, selecting an entry in the dropdown list will diplay the corresponding
                                field's value for that entry in the text box.  If an int is provided, columns are chosen 
                                by index (starting with 0).  If not set, the field specified by SearchField will be used.""")
    ...
    ColNames = property(_getColNames, _setColNames, None,
                        """The names used to label columns.     (list of strs)
                                ...
                                blank column names will be added or extra column names ignored as appropriate.""")
...
if __name__ == "__main__":
    ...
    class TestPanel(dabo.ui.dPanel):
	    ...
	    vs.append(dabo.ui.dLabel(self, Caption="Press the down arrow key to see the list of choices.", FontBold=True),
	              alignment='center')
	    vs.appendSpacer(15)
