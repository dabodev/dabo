...
class Serializable(object):
    ...
    def __init__(self, **args):
        ...
        for key, value in args.iteritems():
            assert key in attributeNames, "Unknown attribute name %r for object %s" % (key, self.__class__.__name__)
            self.srcValues[key] = value

    def getExpectedNames(cls):
        assert hasattr(cls, '_xmlSerializationAttributes'), "Class %r does not define list of attributes needed for deserialization" % cls
        names = []
    ...
    def evaluate(self, env, onlyAttr=None):
        for attrName, attrType in self._xmlSerializationAttributes:
            ...
            try:
                ...
                import traceback
                traceback.print_exc()
                raise Exception("Error validating value %r for attribute %r (type %r) in object %s" % (
                                    value, attrName, attrType.__class__.__name__, self.__class__.__name__))
