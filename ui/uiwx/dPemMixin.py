...
class dPemMixin(dPemMixinBase):
	...
	def unlockDisplay(self):
		...
		try:
			...
			dabo.logError(_("Extra call to unlockDisplay() for object %s") % self)
...
#- 			dabo.ui.callAfterInterval(300, self.refresh)
				...
				try:
					...
					try:
						...
						try:
							...
							dabo.logError(_("Invalid MousePointer value: '%s'") % val)
							return
	...
	def _setRegID(self, val):
		...
		try:
			...
			dabo.logError(_("Failed to register RegID '%s'") % val)
	...
	def _getVisible(self):
		try:
			...
			dabo.logError(_("The object %s does not support the Visible property.") % self)
			return None
